...
export async function initializeUserData(userId: string) {
  try {
    ...
    const { error } = await (supabase as any).from('categories').insert(defaultCategories);
    if (error) {
...
export const goalsService = {
  ...
  async createGoal(goal: Omit<Goal, "id" | "created_at" | "updated_at">) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error("Usuário não autenticado");
    const { data, error } = await db.from("goals").insert({ ...goal, user_id: user.id }).select().single();
    if (error) throw error;
    return data as Goal;
  },
...
export const financesService = {
  async fetchFinances() {
    const { data, error } = await db.from("finances").select("*").order("date", { ascending: false });
    if (error) throw error;
    return data as Finance[];
  },
  async createFinance(finance: Omit<Finance, "id" | "created_at" | "updated_at">) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error("Usuário não autenticado");
    const { data, error } = await db.from("finances").insert({ ...finance, user_id: user.id }).select().single();
    if (error) throw error;
    return data as Finance;
  },
...
export const habitsService = {
  ...
  async createHabit(habit: Omit<Habit, "id" | "created_at" | "updated_at">) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error("Usuário não autenticado");
    const { data, error } = await db.from("habits").insert({ ...habit, user_id: user.id }).select().single();
    if (error) throw error;
    return data as Habit;
  },
...
export const remindersService = {
  async fetchReminders() {
    const { data, error } = await db.from("reminders").select("*").order("reminder_date", { ascending: true });
    if (error) throw error;
    return data as Reminder[];
  },
  async createReminder(reminder: Omit<Reminder, "id" | "created_at" | "updated_at">) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error("Usuário não autenticado");
    const { data, error } = await db.from("reminders").insert({ ...reminder, user_id: user.id }).select().single();
    if (error) throw error;
    return data as Reminder;
  },
...
export const savingsService = {
  async fetchSavings() {
    const { data, error } = await db.from("savings").select("*").order("created_at", { ascending: false });
    if (error) throw error;
  ...
  async createSaving(saving: Omit<Saving, "id" | "created_at" | "updated_at">) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error("Usuário não autenticado");
    const { data, error } = await db.from("savings").insert({ ...saving, user_id: user.id }).select().single();
    if (error) throw error;
    return data as Saving;
  },
  async updateSaving(id: string, updates: Partial<Saving>) {
    const { data, error } = await db.from("savings").update(updates).eq("id", id).select().single();
    if (error) throw error;
    return data as Saving;
  },
  async deleteSaving(id: string) {
    const { error } = await db.from("savings").delete().eq("id", id);
    if (error) throw error;
  },
};
